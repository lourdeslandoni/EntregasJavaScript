...
function mostrarProductosComerciales()
{
    ...
    subtituloH2.innerHTML = "Listado de productos comerciales cargados";
...
{
    const subtituloH2 = document.createElement("h2");
    subtituloH2.innerHTML = "Resultado de productos comerciales filtrados por marca";
    document.body.appendChild(subtituloH2);

    let marca = prompt("Ingresa la marca comercial del producto que quieras buscar");
    //Si uso el metodo FIND, me devuelve un objeto {}, si uso el metodo FILTER, me devuelve un array []
    let encontrados = productosComerciales.filter((productoComercial)=>productoComercial.marcaComercial.toLowerCase().indexOf(marca.toLocaleLowerCase())!==-1);

    let productosEncontrados = document.querySelector("#productosEncontrados");
    if(!productosEncontrados)
    {
        productosEncontrados=document.createElement("ul");
        productosEncontrados.setAttribute("id", "productosEncontrados");
    }
    productosEncontrados.innerHTML="";

    encontrados.forEach((productoComercial)=>{
        const nodoLI=document.createElement("li");
        nodoLI.innerHTML=`<b>Marca Comercial:</b>${productoComercial.marcaComercial}<br> 
                          <b>Denominacion de venta:</b>${productoComercial.denominacionDeVenta}<br>
                          <b>Pais de Origen:</b>${productoComercial.paisDeOrigen}<br>
                          ...
                          <br>
                          `;
        productosEncontrados.appendChild(nodoLI);
   });
   document.body.appendChild(productosEncontrados);
}

function filtrarProductosPorIngrediente()
{
    const subtituloH2 = document.createElement("h2");
    subtituloH2.innerHTML = "Resultado de productos comerciales filtrados por ingrediente";
    document.body.appendChild(subtituloH2);
    ...
    let ingredientesEncontrados = document.querySelector("#ingredientesEncontrados");
    if(!ingredientesEncontrados)
    {
        ingredientesEncontrados=document.createElement("ul");
        ingredientesEncontrados.setAttribute("id", "ingredientesEncontrados");
    }
    ingredientesEncontrados.innerHTML="";
    ...
    encontrados.forEach((productoComercial)=>{
        ...
        nodoLI.innerHTML=`<b>Marca Comercial:</b>${productoComercial.marcaComercial}<br> 
                          ...
                          <br>
                          `;
        ingredientesEncontrados.appendChild(nodoLI);
   });
   document.body.appendChild(ingredientesEncontrados);
}
...
tituloH1.innerHTML = "Bienvenidxs a la Calculadora de Sellos y Simulador de Formulaciones de Analogos Lacteos";
